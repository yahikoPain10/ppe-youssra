/* =========================================================
   ÉPI v237 — Interface finale propre : modification par composants réels
...
(function(){
  ...
  function normalizeChildren(value){
    ...
    return rows.map(item => {
      if(item && typeof item === 'object'){
        return {
          title: text(item.title || item.name || item.text || item.label || item.value || ''),
          definition: text(item.definition || item.description || item.detail || '')
        };
      }
  ...
  function collectFormChildren(formEl){
    return $$('[data-v235-child-row]', formEl).map(row => ({
      title: text($('[data-child-field="title"]', row)?.value || ''),
      definition: text($('[data-child-field="definition"]', row)?.value || '')
  ...
  function visualFromLesson(lesson, subtitle){
    const out = [];
    try{
      const media = (typeof visualMediaForLesson === 'function') ? (visualMediaForLesson(lesson) || []) : ((window.LESSON_MEDIA && lesson && window.LESSON_MEDIA[lesson.title]) || []);
      (Array.isArray(media) ? media : []).forEach((m, i) => {
        let detail = {};
        try{ if(typeof visualExampleFor === 'function') detail = visualExampleFor(m, lesson, i) || {}; }catch(e){}
        out.push(normalVisual({
          title: detail.title || m.title || subtitle || `Image ${i+1}`,
          src: m.src || m.url || m.image || '',
          description: detail.definition || m.caption || m.description || '',
          example: detail.example || m.example || '',
          subtitleTitle: subtitle || (lesson && lesson.title) || ''
        }, i, subtitle));
      });
    }catch(e){}
    if(!out.length && lesson && lesson.visual && (lesson.visual.image || lesson.visual.src)){
      out.push(normalVisual({title: lesson.title || subtitle, src: lesson.visual.image || lesson.visual.src, description: lesson.fr || lesson.objective || '', subtitleTitle: subtitle}, 0, subtitle));
    }
    return out;
  }

  function audioFromLesson(lesson, subtitle){
    const out = [];
    function push(item, i){
      if(typeof item === 'string') out.push(normalAudio({title: subtitle || `Audio ${i+1}`, text:item, subtitleTitle:subtitle}, i, subtitle));
      else if(item) out.push(normalAudio({title:item.title || subtitle || `Audio ${i+1}`, src:item.src || item.url || '', text:item.fr || item.text || item.definition || '', ar:item.ar || '', subtitleTitle:subtitle}, i, subtitle));
    }
    try{
      let rows = [];
      if(typeof tableurAudioItems === 'function') rows = tableurAudioItems(lesson) || [];
      if(!rows.length && typeof logoAudioItems === 'function') rows = logoAudioItems(lesson) || [];
      if(!rows.length && typeof audioScriptItems === 'function') rows = (audioScriptItems(lesson) || []).map((t, i) => ({title:`Audio ${i+1}`, text:t}));
      rows.forEach(push);
    }catch(e){}
    if(!out.length && lesson && (lesson.fr || lesson.ar || lesson.objective)) out.push(normalAudio({title: subtitle || lesson.title || 'Audio', text: lesson.fr || lesson.objective || '', ar: lesson.ar || '', subtitleTitle: subtitle}, 0, subtitle));
    return uniqueRows(out);
  }

  function kineFromLesson(lesson, subtitle){
    const out = [];
    try{
      if(typeof genericPracticeItems === 'function'){
        const gp = genericPracticeItems(lesson) || {};
        (Array.isArray(gp.questions) ? gp.questions : []).forEach((q, i) => out.push(normalKine({
          type: 'Question directe',
          question: q.prompt || q.question || q.task || '',
          options: Array.isArray(q.choices) ? q.choices.join('\n') : (q.options || ''),
          answer: q.answer || q.expected || '',
          correction: q.correction || q.answer || q.expected || '',
          subtitleTitle: subtitle
        }, i, subtitle)));
      }
    }catch(e){}
    if(lesson && Array.isArray(lesson.fill) && lesson.fill[0]) out.push(normalKine({type:'Compléter', question:lesson.fill[0], options:Array.isArray(lesson.fill[2]) ? lesson.fill[2].join('\n') : '', answer:lesson.fill[1] || '', correction:lesson.fill[1] || '', subtitleTitle:subtitle}, out.length, subtitle));
    if(lesson && Array.isArray(lesson.drag)){
      lesson.drag.slice(0,3).forEach(pair => { if(Array.isArray(pair)) out.push(normalKine({type:'Association', question:`Associe « ${pair[0] || ''} » à sa définition.`, answer:pair[1] || '', correction:pair[1] || '', subtitleTitle:subtitle}, out.length, subtitle)); });
    }
    if(lesson && lesson.order && Array.isArray(lesson.order.steps)) out.push(normalKine({type:'Ordre logique', question:lesson.order.title || 'Remets les étapes dans l’ordre.', options:lesson.order.steps.join('\n'), answer:lesson.order.steps.join(' → '), correction:lesson.order.steps.join(' → '), subtitleTitle:subtitle}, out.length, subtitle));
    return uniqueRows(out);
  }
  ...
  function derive(unit){
    ...
    if(!subtitles.length){
      subtitles = lessons.map((l, i) => ({
        ...
        children: [],
        visualItems: visualFromLesson(l, l.title || `Sous-titre ${i+1}`),
        audioItems: audioFromLesson(l, l.title || `Sous-titre ${i+1}`),
        kineExercises: kineFromLesson(l, l.title || `Sous-titre ${i+1}`)
      }));
    }
    ...
    subtitles = subtitles.map((item, i) => {
      ...
      const savedHasSubtitles = Array.isArray(s.subtitleItems);
      const canUseBaseVisual = !savedHasSubtitles && !Array.isArray(s.visualItems);
      const canUseBaseAudio = !savedHasSubtitles && !Array.isArray(s.audioItems);
      const canUseBaseKine = !savedHasSubtitles && !Array.isArray(s.kineExercises) && !Array.isArray(s.kineQuestions);
      const visualItems = uniqueRows([].concat(
        ...
        canUseBaseVisual ? visualFromLesson(lesson, title) : []
      ));
      const audioItems = uniqueRows([].concat(
        ...
        canUseBaseAudio ? audioFromLesson(lesson, title) : []
      ));
      ...
      if(!kineExercises.length && canUseBaseKine) kineExercises = kineFromLesson(lesson, title);
      return {
        title,
  ...
  function flatten(c){
    ...
    const lessons = subtitleItems.map((sub, i) => ({
      title: sub.title || `Sous-titre ${i+1}`,
      competence: c.competence || '',
      officialFocus: sub.title || `Sous-titre ${i+1}`,
      objective: sub.definition || '',
      officialObjectives: lines(sub.children),
      visualType: 'mindmap',
      visual: { image: (sub.visualItems && sub.visualItems[0] && sub.visualItems[0].src) || '', items: [], center: sub.title || `Sous-titre ${i+1}` },
      fr: sub.definition || '',
      ar: '',
      fill: [((sub.kineExercises || [])[0] || {}).question || 'Question : ____', ((sub.kineExercises || [])[0] || {}).answer || '', lines(((sub.kineExercises || [])[0] || {}).options || '')],
      drag: [],
      order: { title: sub.title || `Sous-titre ${i+1}`, steps: [] },
      visualItems: Array.isArray(sub.visualItems) ? sub.visualItems : [],
      audioItems: Array.isArray(sub.audioItems) ? sub.audioItems : [],
      kineExercises: Array.isArray(sub.kineExercises) ? sub.kineExercises : []
    }));
    const examQuestions = Array.isArray(c.examQuestions) ? c.examQuestions : [];
    const dictionary = Array.isArray(c.dictionary) ? c.dictionary : [];
    const integrationTasks = Array.isArray(c.integrationTasks) ? c.integrationTasks : [];
    return Object.assign({}, c, {
      ...
      dictionary,
      integration: {title:c.integrationTitle || 'Situation d’intégration', scenario:c.scenario || '', tasks:integrationTasks.map(t=>t.task || ''), questions:integrationTasks.map(t=>[t.task || '', t.expected || ''])},
      integrationTasks,
      examQuestions,
      exam: examQuestions.map(q => [q.question || '', Array.isArray(q.options) ? q.options : lines(q.options || ''), Number(q.correct || 0)]),
      savedAt: new Date().toISOString(),
      savedBy: 'v237-final-composants'
    });
  }
  function save(){
    const u = unitById(activeUnitId);
    if(!u || !activeData) return false;
    const payload = flatten(activeData);
    const ok = writeJson(keyOf(u), payload);
    if(ok){
      try{
        Object.assign(u, {
          title: payload.title || u.title,
          short: payload.short || u.short,
          intro: payload.intro || u.intro,
          duration: payload.duration || u.duration,
          competence: payload.competence || u.competence,
          schoolYear: payload.schoolYear || u.schoolYear,
          lessons: payload.lessons,
          mindmapBranches: payload.mindmapBranches,
          dictionary: payload.dictionary.map(d => [d.term || '', d.value || '']),
          integration: payload.integration,
          exam: payload.exam,
          _v170TeacherContent: payload,
          _v172TeacherContent: payload
        });
      }catch(e){}
    }
    return ok;
  ...
  function renderHero(c){
    return `<section class="v235-unit-head">
      <div><span>Unité sélectionnée</span><h2>${esc(c.title || 'Unité')}</h2></div>
  ...
  function injectCss(){
    ...
    st.textContent = `
      ...
      /* V237 — couleurs plus simples et interface moins lourde */
      .v235-editor{background:#f6f1ff!important;min-height:auto!important;border:1px solid rgba(124,58,237,.18)!important;box-shadow:none!important;}
      .v235-toolbar{position:relative!important;top:auto!important;grid-template-columns:1fr minmax(260px,520px) auto auto!important;max-width:980px!important;margin:0 auto!important;width:100%!important;background:#fff!important;border:1px solid rgba(124,58,237,.22)!important;box-shadow:0 8px 18px rgba(76,29,149,.08)!important;}
      .v235-toolbar strong{text-align:left!important;color:#4c1d95!important;}
      .v235-toolbar select{justify-self:center!important;text-align:center!important;}
      .v235-body{max-width:980px!important;}
      .v235-unit-head,.v235-section,.v235-subtitle,.v235-style{box-shadow:none!important;border-color:rgba(124,58,237,.18)!important;}
      .v235-unit-head p,.v235-alert{display:none!important;}
      .v176-panel-head[data-v237-hidden], [data-v176-panel="modify"]>.v176-panel-head{display:none!important;}
      .v236-unit-page{width:min(900px, calc(100vw - 28px))!important;margin:8px auto 18px!important;}
      .v236-unit-selector{position:relative!important;top:auto!important;grid-template-columns:1fr!important;max-width:570px!important;margin:0 auto 10px!important;padding:8px 10px!important;background:#fff!important;border:1px solid rgba(124,58,237,.24)!important;box-shadow:0 6px 14px rgba(76,29,149,.08)!important;}
      .v236-unit-selector-title{display:none!important;}
      .v236-select-wrap{max-width:540px!important;margin:0 auto!important;width:100%!important;text-align:center!important;}
      .v236-select-wrap span{display:none!important;}
      .v236-select-wrap select{min-height:38px!important;text-align:center!important;font-size:14px!important;border-width:1px!important;}
      .v236-content-card,.v236-unit-page .content-card{max-width:860px!important;margin:0 auto!important;padding:9px!important;}
      .v236-unit-page .panel,.v236-unit-page section.panel,.v236-unit-page .official-box,.v236-unit-page .teacher-sheet{padding:8px!important;margin-bottom:7px!important;border-radius:14px!important;}
      .v236-unit-page h2{font-size:1.02rem!important}.v236-unit-page h3{font-size:.94rem!important}.v236-unit-page p,.v236-unit-page li{font-size:.86rem!important;line-height:1.28!important}.v236-unit-page img{max-height:118px!important;}
      .v121-about,.v121-about.v146-about{right:18px!important;left:auto!important;bottom:16px!important;width:min(390px,calc(100vw - 36px))!important;text-align:center!important;justify-content:center!important;align-items:center!important;}
      .v121-about *{text-align:center!important;}
      @media(max-width:880px){.v235-toolbar{grid-template-columns:1fr!important}.v235-toolbar strong{text-align:center!important}.v236-unit-page{width:min(100%,calc(100vw - 12px))!important}.v121-about,.v121-about.v146-about{right:10px!important;left:10px!important;width:auto!important;}}
  ...
  function install(){
    injectCss();
    document.body.classList.add('v235-ready','v237-ready');
